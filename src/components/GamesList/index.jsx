...
function GamesList() {
  ...
  const sideClass = classNames("games__side", {
    "games__side--openned": menuToggled,
  });
  const orderFilterClass = classNames("games__side__filter__sorting__btn", {
    "games__side__filter__sorting__btn--disable": !checkFilter,
  });
  const CheckIconFilter = !checkFilter
    ? AiOutlineCheckCircle
    : AiOutlineCloseCircle;
  ...
  const handleDusting = (name) => {
    ...
    !checkFilter && !selectedTime && !selectedPlayer
      ? dispatch(dustAll())
      : dispatch(dustBy(gamesList));
  };
    ...
    if (!selectedTime) {
      ...
      switch (selectedTime) {
        ...
        case 20:
          found = games.filter(
            (element) =>
              element.game.play_time >= 15 && element.game.play_time <= 24
          );
          break;
        case 30:
          found = games.filter(
            (element) =>
              element.game.play_time >= 25 && element.game.play_time <= 39
          );
          break;
        case 45:
          found = games.filter(
            (element) =>
              element.game.play_time >= 40 && element.game.play_time <= 54
          );
          break;
        case 60:
  ...
  const sortGamesPlayer = (games) => {
    if (!selectedPlayer) {
      ...
      const found = games.filter(
        (element) =>
          parseInt(element.game.min_player, 10) <=
            parseInt(selectedPlayer, 10) &&
          parseInt(element.game.max_player, 10) >= parseInt(selectedPlayer, 10)
      );
      return found;
    }
  };
  ...
  if (checkFilter) {
    if (selectedTime) {
      gamesList = sortGamesOrder(
        sortGamesTime(filterGamesBySearch(checkedGames, searchInput))
      );
    } else if (selectedPlayer) {
      gamesList = sortGamesOrder(
        sortGamesPlayer(filterGamesBySearch(checkedGames, searchInput))
      );
      ...
      gamesList = sortGamesOrder(
        filterGamesBySearch(checkedGames, searchInput)
      );
    }
  } else if (selectedTime) {
    if (selectedPlayer) {
      gamesList = sortGamesTime(
        sortGamesPlayer(filterGamesBySearch(checkedGames, searchInput))
      );
    } else {
    ...
    <div className="games">
              ...
              <div
                ...
                className={
                  checkFilter
                    ? ite.checked
                      ? "games__list__game"
                      : "games__list__game games__list__game--checked"
                    : "games__list__game"
                }
                onClick={() => {
                  !checkFilter
                    ? handleGameSelection(ite.game.id)
                    : dispatch(checkGame(ite.game.id));
                }}
              >
                <img
      ...
      <div className={sideClass}>
        ...
        <div className="games__side__btns">
          <div
            className="games__side__btn"
            onClick={() => dispatch(toggleModal("addgames"))}
          >
            <AiOutlineAppstoreAdd className="games__side__btn__icon" />
            <div className="games__side__btn__name">Ajouter un jeu</div>
          </div>
          <div
            className="games__side__btn games__side__btn--filter"
        ...
        <div className="games__side__filter">
          <div className="games__side__filter__btns">
            ...
            <BiFilterAlt
              className="games__side__filter__btn games__side__filter__btn--filter"
              onClick={() => dispatch(toggleFilterMenu())}
            />
          ...
          <div className="games__side__filter--scroll">
            <div className="games__side__filter__sorting">
              <CheckIconFilter
                className={`games__side__filter__sorting__btn ${
                  checkFilter && "games__side__filter__sorting__btn--active"
                }`}
                onClick={handleCheckingGames}
              />
              <AlpaIconFilter
                className={orderFilterClass}
                onClick={() =>
                  dispatch(toggleFilter("sortAlphaFilter", !sortAlphaFilter))
                }
              />
              <SortkIconFilter
                className={orderFilterClass}
                onClick={() =>
                  dispatch(toggleFilter("sortNumFilter", !sortNumFilter))
                }
              />
              <BiSort
                className={`${orderFilterClass} ${
                  sortDirFilter && "games__side__filter__sorting__btn--active"
                }`}
                onClick={() =>
                  dispatch(toggleFilter("sortDirFilter", !sortDirFilter))
                }
              />
            </div>
            <Field
            ...
            <div className="games__side__filter__ctn">
              <div
                className="games__side__filter__type"
                onClick={() =>
                  dispatch(toggleFilter("timesFilter", !timesFilter))
                }
              >
                <BsHourglassSplit className="games__side__filter__type__icon" />
                <div className="games__side__filter__type__name">Durée</div>
                {timesFilter ? (
                  <AiFillCaretUp className="games__side__filter__type__icon" />
                ) : (
                  <AiFillCaretDown className="games__side__filter__type__icon" />
                )}
              </div>
              {timesFilter && (
                <div className="games__side__filter__type__items">
                  {selectedTime !== 10 ? (
                    <div
                    ...
                    <AiOutlineCloseCircle
                      ...
                      onClick={() =>
                        handleFilterSelect("selectedTime", null, true)
                      }
                    />
                  )}
                  {selectedTime !== 20 ? (
            ...
            <div className="games__side__filter__ctn">
              <div
                className="games__side__filter__type"
                onClick={() =>
                  dispatch(toggleFilter("playersFilter", !playersFilter))
                }
              >
              ...
              {playersFilter && (
                <form className="games__side__filter__type__items">
                  <Field
                  ...
                  />
                  <IoIosBackspace
                    className="games__side__filter__type__item--back"
                    onClick={() =>
                      handleFilterSelect("selectedPlayer", "", true)
                    }
                  />
                </form>
              )}
