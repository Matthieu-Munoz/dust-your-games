...
// Utilities
import { setWithExpiry, getWithExpiry } from "src/utils/localStorage";
...
const dygApiMiddleWare = (store) => (next) => (action) => {
  switch (action.type) {
    ...
    case REGISTER: {
      store.dispatch(toggleLoading(true));
      const {
        home: { pseudo_name, birthday, email, password },
      } = store.getState();
      ...
      axiosInstance
        ...
        .then((response) => {
          ...
          if (response.status === 200) {
            store.dispatch(toggleLoading(false));
            store.dispatch(
              sendAlert("error", `Cet utilisateur est déjà enregistré.`)
            );
            setTimeout(() => {
              store.dispatch(closeAlert());
            }, 2800);
        ...
        .catch(() => {
          store.dispatch(toggleLoading(false));
          store.dispatch(
            sendAlert("error", "Une erreur est survenue. Veuillez réessayer.")
          );
          setTimeout(() => {
            store.dispatch(closeAlert());
          }, 2800);
        ...
        .then((response) => {
          if (response.status === 200) {
            ...
            store.dispatch(
              sendAlert(
                "check",
                `Connexion réussie : bienvenue ${response.data.user.pseudo_name}`
              )
            );
            store.dispatch(toggleLoading(false));
        ...
        .then((response) => {
          if (response.status === 202) {
            ...
            store.dispatch(
              sendAlert(
                "check",
                `Vos nouveaux identifiant vous ont été envoyés.`
              )
            );
            setTimeout(() => {
              store.dispatch(closeAlert());
    ...
    case EDIT_USER: {
      store.dispatch(toggleLoading(true));
      const {
        account: { pseudo_name, image, email, password },
      } = store.getState();
      const {
        user: { id },
      } = store.getState();
      axiosInstance
        .patch(`${id}/profil/edit`, {
          email,
          pseudo_name,
          password,
          image,
        ...
        .then((response) => {
          if (response.status === 201) {
            ...
            store.dispatch(saveUserAccount(user));
            store.dispatch(
              sendAlert("check", `Votre profil a bien été modifié.`)
            );
            setTimeout(() => {
              store.dispatch(closeAlert());
            }, 2800);
      ...
      axiosInstance
        ...
        .then((response) => {
          if (response.status === 202) {
            ...
            localStorage.removeItem("user");
            store.dispatch(
              sendAlert("check", `Votre profil a bien été supprimé.`)
            );
            store.dispatch(loginConfirm(false));
            setTimeout(() => {
              store.dispatch(closeAlert());
            ...
            response.data.forEach((currentGame) => {
              currentGame.game.category.forEach((currentCategory) => {
                const found = category.find(
                  (element) => element.name === currentCategory.name
                );
                if (!found) {
      ...
      axiosInstance
        .post(`${id}/add`, {
          ...
          image: game[0].image_url
            ? game[0].image_url
            : "https://res.cloudinary.com/dyg/image/upload/v1653338209/t%C3%A9l%C3%A9chargement_whni6b.png",
          ...
          editor: {
            name:
              game[0].primary_publisher && game[0].primary_publisher.name
                ? game[0].primary_publisher.name
                : "Inconnu",
          },
          designor: {
            name:
              game[0].primary_designer && game[0].primary_designer.name
                ? game[0].primary_designer.name
                : "Inconnu",
          },
        ...
        .then((response) => {
          if (response.status === 201) {
            ...
            store.dispatch(
              sendAlert("check", `Ce jeu a bien été ajouté à votre liste`)
            );
            setTimeout(() => {
              store.dispatch(closeAlert());
            }, 2800);
          ...
          if (response.status === 200) {
            ...
            store.dispatch(selectSearchGame(null));
            store.dispatch(
              sendAlert("error", `Ce jeu est déjà dans votre collection`)
            );
            setTimeout(() => {
              store.dispatch(closeAlert());
            }, 2800);
    ...
    case DELETE_GAME: {
      ...
      axiosInstance
        .delete(`${id}/games/${selectedGame}/delete`)
        .then((response) => {
          if (response.status === 202) {
            const user = {
              nb_games: nb_games - 1,
      ...
      axiosInstance
        ...
        .then((response) => {
          if (response.status === 200) {
            store.dispatch(fetchGames());
            store.dispatch(
              sendAlert("check", `Le jeu a bien été dépoussiéré !`)
            );
            setTimeout(() => {
              store.dispatch(closeAlert());
            }, 2800);
